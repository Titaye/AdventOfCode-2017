...
let puzzleInput = """bdwdjjo avricm cjbmj ran lmfsom ivsof
...
let extractWords =
  let regex = Regex(@"[a-z]+", RegexOptions.Compiled)
  regex.Matches
  >> Seq.cast<Match>
...
let isAnagram (w1:string) (w2:string) =
  ...
  | false -> false
  | true ->
      let rec isAnagram i (remainingChars:List<char>) =
        let c = w1.[i]
        match remainingChars.Remove(c) with
        | false -> false
        | true when i = w1.Length - 1 -> true
        | true -> isAnagram (i + 1) remainingChars
      isAnagram 0 (List(w2))

let validatePassphrase cond (words:string seq) =
  let wordsAndIndexes =
    words
  ...
  wordsAndIndexes
  |> Array.tryFind (fun (i,x) -> 
      wordsAndIndexes 
      |> Array.tryFind(fun (j,y) -> i <> j && cond x y )
      |> Option.isSome )
  |> Option.isNone
  ...
  testList "day04" [
    testList "puzzle1" [
      ...
      testCase "puzzle answer" <| fun _ ->
        ...
        puzzleInput.Split '\n'
        |> Array.filter (extractWords >> validatePassphrase (=))
        |> Array.length
        |> printfn "Day04 puzzle1 answer: %i "
    ]
    ...
    testList "puzzle2" [
      ...
      testCase "puzzle answer" <| fun _ ->
        puzzleInput.Split '\n'
        |> Array.filter (extractWords >> validatePassphrase isAnagram)
        |> Array.length
        |> printfn "Day04 puzzle2 answer: %i "
