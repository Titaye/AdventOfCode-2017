...
let parseRelations =
  let regex = Regex(@"(?<parent>\w+) \(\d+\) -> ((?<children>\w+)(, )?)+", RegexOptions.Compiled)
  let captureValue (c:Capture) = c.Value
  let capturesOfGroup (name:string) (m:Match) =
    m.Groups.[name].Captures
    |> Seq.cast<Capture>
    |> Seq.map captureValue 
  ...
  >> Seq.map( fun m ->
      let parent = 
        m 
        |> capturesOfGroup "parent"
        |> Seq.head
      let children = 
        m 
        |> capturesOfGroup "children"
        |> Seq.toList
...
let parseWeights =
  let regex = Regex(@"(?<node>\w+) \((?<weight>\d+)\)", RegexOptions.Compiled)
  let captureValue (c:Capture) = c.Value
  let capturesOfGroup (name:string) (m:Match) =
    m.Groups.[name].Captures
    |> Seq.cast<Capture>
  ...
  >> Seq.map( fun m ->
      let node = 
        m 
        |> capturesOfGroup "node"
        |> Seq.head
      let weight = 
        m 
        |> capturesOfGroup "weight"
        |> Seq.map Int32.Parse
        |> Seq.head
...
let findRootOfRelations relations =
  let allChildren =
    relations
    |> List.collect snd
    |> Set.ofList
  ...
  relations
  |> Seq.find (fun (parent,_) -> allChildren |> Set.contains parent |> not)

let findRootOfRelationsString =
  parseRelations >> findRootOfRelations
  
let findUnbalancedWeight input =
  let relations = 
    input 
    |> parseRelations
    
  let relationsMap = relations |> Map.ofList
  ...
  let root = relations |> findRootOfRelations
  ...
  let bindItems (f:'a->Result<'b,'c>) (items:'a list) :  Result<'b list,'c> =
    let rec processItems res = function
      | [] -> res |> Result.Ok
      | h :: tail -> 
          f h |> function
          | Error e -> Result.Error e
          | Ok r ->  processItems (r :: res) tail
    items |> processItems []


  
  let rec checkNodesBalance node =
    match node |> getChildren with
    | [] -> (node, node |> getNodeValue) |> Result.Ok
    | children -> 
        match children |> bindItems checkNodesBalance with
        | Error e -> Error e
        | Ok r -> 
            let grp = r |> List.groupBy (fun (_node,value) -> value)
            if grp.Length = 1 then
              // all children are balanced
              let value = (node |> getNodeValue) + (r |> List.map snd |> List.sum) 
              (node, value) |> Result.Ok
            else
              // Assuming only one unbalanced node and more than 2 children per node, the expected value is the value where more than one node
              let unbalancedNode =
                grp 
                |> List.find ( fun (_value,items) -> items |> List.length = 1)
                |> snd 
                |> List.head

              let firstBalancedNode =
                grp 
                |> List.find ( fun (_value,items) -> items |> List.length > 1)
                |> snd 
                |> List.head

              // printfn "unbalancedNode %A" unbalancedNode
              let unbalancedNodeValue = (unbalancedNode |> fst |> getNodeValue)
              // printfn "unbalancedNode value %A" unbalancedNodeValue
              // printfn "firstBalancedNode %A" firstBalancedNode

              let delta = (unbalancedNode |> snd) - (firstBalancedNode |> snd)
              let correctedValue = (unbalancedNode |> fst |> getNodeValue) - delta
              // printfn "delta %A" delta
              
              correctedValue |> Result.Error        

        
  checkNodesBalance (root |> fst)
...
gyxo (61)
cntj (57)"""
        let (parent, _) = input |> findRootOfRelationsString
        
        Expect.equal parent "tknk" "Puzzle input not validated"
      ...
      testCase "puzzle answer" <| fun _ ->
        ...
        let (parent, _) = input |> findRootOfRelationsString
        parent
        |> printfn "Day07 puzzle1 answer: %A "
    ]
...
gyxo (61)
cntj (57)"""
        let r = input |> findUnbalancedWeight
        
        Expect.equal r (Error 60) "Puzzle input not validated"


      testCase "puzzle answer" <| fun _ ->
        let input = puzzleInput
        let result = input |> findUnbalancedWeight
        result
        |> printfn "Day07 puzzle2 answer: %A "
