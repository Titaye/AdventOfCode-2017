...
type Registers = ConcurrentDictionary<string,int>
type Instruction = Instruction of register:string * incValue:int * test:(Registers -> bool)

let puzzleInput = """sdj dec 558 if r <= 8
...
let parseInstructions =
  let regex = Regex(@"(?<reg>\w+) (?<op>(inc|dec)) (?<v>-?\d+) if (?<testreg>\w+) (?<testop>(\<|\>|\>=|\<=|==|!=)) (?<testval>-?\d+)", RegexOptions.Compiled)
  let captureValue (c:Capture) = c.Value
  let capturesOfGroup (name:string) (m:Match) =
    m.Groups.[name].Captures
    |> Seq.cast<Capture>
    |> Seq.map captureValue
  let captureOfGroup name = capturesOfGroup name >> Seq.head
    
  let createTest testRegName testOp testValue (registers:Registers) =
    let v = registers.GetOrAdd(testRegName, Func<_,_>(fun _ -> 0))
    match testOp with
    | "<" -> v < testValue
    | ">" -> v > testValue
    | ">="-> v >= testValue
    ...
    | "=="-> v = testValue
    | "!="-> v <> testValue
    | _ -> failwith "invalidOperator"

  regex.Matches
  >> Seq.cast<Match>
  >> Seq.map( fun m ->
      let reg = m |> captureOfGroup "reg"
      let op = m |> captureOfGroup "op"
      let v = m |> captureOfGroup "v" |> Int32.Parse
      let testReg = m |> captureOfGroup "testreg"
      let testOp = m |> captureOfGroup "testop"
      let testVal = m |> captureOfGroup "testval" |> Int32.Parse
      ...
      Instruction (reg, instructionValue, createTest testReg testOp testVal))
  >> Seq.toList

let processProgram input =
  let instructions = 
    input
    |> parseInstructions
  let registers = ConcurrentDictionary<string,int>()
  let mutable highestValue = None

  instructions
  |> List.iter (fun (Instruction (reg, value, cond)) ->
      if registers |> cond then
        let applyValue _k  existingValue = existingValue + value               
        let newVal = registers.AddOrUpdate(reg, value, Func<_,_,_>  applyValue)
        highestValue <- 
          match highestValue with
          | None -> Some (reg, newVal)
          | Some (_regName, v) ->
              if newVal > v then Some (reg, newVal)
              else highestValue )

  registers |> Seq.maxBy (fun x -> x.Value), highestValue
...
c dec -10 if a >= 1
c inc -20 if c == 10"""
        let result = input |> processProgram |> fst
        
        Expect.equal result.Value 1 "Puzzle input not validated"


      testCase "puzzle answer" <| fun _ ->
        let input = puzzleInput
        let result = input |> processProgram
        result
        |> printfn "Day08 puzzle1 answer: %A "
    ]
...
c dec -10 if a >= 1
c inc -20 if c == 10"""
        let result = input |> processProgram |> snd
        
        Expect.equal result (Some ("c", 10)) "Puzzle input not validated"
      ...
      testCase "puzzle answer" <| fun _ ->
        ...
        let result = input |> processProgram
        result
        |> printfn "Day08 puzzle2 answer: %A "
    ]
  ]